...
class Client:
	...
	def listenRtp(self):		
		...
		while True:
			try:
				...
				# Get time after receive a frame
				...
				if data:
					...
					if currFrameNbr > self.statHighSeqNb:
						self.statHighSeqNb = currFrameNbr
					...
					if self.statTotalPlayTime == 0:
						...
						self.statDateRate = self.statTotalBytes / (self.statTotalPlayTime / 1000.0)
					self.statFractionLost = self.statCumLost / self.statHighSeqNb
					self.statTotalBytes = self.statTotalBytes + len(rtpPacket.getPayload())
			except:
	...
	def sendRtspRequest(self, requestCode):
		...
		# Extend: Question 1
		print ("\n===================================")
		print ("Total Bytes Recevied: " + str(self.statTotalBytes))
		print ("RTP Packet Loss Rate: {FractionLost:.2f}".format(FractionLost = self.statFractionLost))
		print ("Video Data Rate: {DateRate:.2f}".format(DateRate = self.statDateRate) + " bytes/s")
		print ("===================================\n")
	...
	def handler(self):
		...
		# When the user presses cancel, resume state
		else:
